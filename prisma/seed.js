...
async function main() {
  ...
  const subareas = [
    { name: 'Chueca', cityName: 'Madrid', latitude: 40.4223, longitude: -3.6986 },
    { name: 'Eixample', cityName: 'Barcelona', latitude: 41.388, longitude: 2.161 },
    { name: 'Malasaña', cityName: 'Madrid', latitude: 40.4282, longitude: -3.7045 },
    { name: 'Ruzafa', cityName: 'Valencia', latitude: 39.4605, longitude: -0.3763 },
    { name: 'El Carmen', cityName: 'Valencia', latitude: 39.4765, longitude: -0.3781 },
    { name: 'La Malagueta', cityName: 'Málaga', latitude: 36.7204, longitude: -4.4137 },
    { name: 'Centro Histórico', cityName: 'Málaga', latitude: 36.7213, longitude: -4.4216 },
    { name: 'Sant Quirze del Vallès', cityName: 'Barcelona', latitude: 41.534, longitude: 2.0823 },
  ];
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const slug = `${slugify(name)}-${faker.string.uuid().slice(0, 8)}`;
    ...
    const girl = await db.girl.create({
      data: {
        ...
        nationality: faker.helpers.arrayElement(['Española', 'Colombiana', 'Argentina']),
